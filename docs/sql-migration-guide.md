...
## Upgrading from Spark SQL 3.3 to 3.4
  ...
  - Since Spark 3.4, `spark.sql.optimizer.canChangeCachedPlanOutputPartitioning` is enabled by default which allows `spark.sql.adaptive.enabled` and `spark.sql.sources.bucketing.autoBucketedScan.enabled` affect cached plan. To restore the previous behavior, set `spark.sql.optimizer.canChangeCachedPlanOutputPartitioning` to `false`.
